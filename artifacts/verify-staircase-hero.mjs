...
for(const width of [1672,1440,834,390,360]){
 ...
 if(!await page.evaluate(()=>document.fonts.check('20px SeoAnton')))throw Error('Font missing');
 if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Overflow '+width);
 await page.locator('.seo-hero').screenshot({path:`artifacts/staircase-hero-${width}.png`});
}
...
console.log('PASS: five viewport sizes, local font, no overflow or runtime errors, enquiry dialog and keyboard focus.');
await browser.close();
